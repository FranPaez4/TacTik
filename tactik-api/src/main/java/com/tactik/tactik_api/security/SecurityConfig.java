...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                .cors(Customizer.withDefaults()) // Habilitamos CORS para permitir peticiones desde el frontend
                .csrf(AbstractHttpConfigurer::disable) // Desactivamos CSRF porque usamos tokens JWT, no cookies
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers(HttpMethod.OPTIONS, "/**").permitAll() // Permitimos todas las peticiones OPTIONS (CORS preflight)
